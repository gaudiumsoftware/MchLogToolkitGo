...
func TestBuildGELFMessageRequiredFields(t *testing.T) {
	...
	msg, err := buildGELFMessage("payments-api", "info", payload, nil, DestinationConfig{
		Source: "pod-1",
	})
	if err != nil {
...
func TestBuildGELFMessageCustomFields(t *testing.T) {
	...
	msg, err := buildGELFMessage("payments-api", "debug", payload, nil, DestinationConfig{
		Source: "pod-1",
	})
	if err != nil {
...
func TestBuildGELFMessageWithError(t *testing.T) {
	...
	msg, err := buildGELFMessage("svc", "error", payload, errors.New("kaboom"), DestinationConfig{
		Source: "pod-1",
	})
	if err != nil {
		t.Fatalf("build failed: %v", err)
...
func TestBuildGELFMessageAcceptsMap(t *testing.T) {
	...
	msg, err := buildGELFMessage("svc", "info", content, nil, DestinationConfig{Source: "pod-1"})
	if err != nil {
		t.Fatalf("build failed: %v", err)
	}
...
func TestBuildGELFMessageMissingMessage(t *testing.T) {
	...
	msg, err := buildGELFMessage("svc", "info", payload, nil, DestinationConfig{Source: "pod-1"})
	if err != nil {
		t.Fatalf("build failed: %v", err)
	}
	if msg.Short != "" {
...
func TestBuildGELFMessageInvalidJSONReturnsError(t *testing.T) {
	...
	if _, err := buildGELFMessage("svc", "info", payload, nil, DestinationConfig{Source: "pod-1"}); err == nil {
