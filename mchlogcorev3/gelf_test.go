...
func TestBuildGELFMessageAcceptsMap(t *testing.T) {
	...
	msg, err := buildGELFMessage("svc", "info", content, nil, NetworkConfig{Type: NetworkGraylogUDP, Source: "pod-1"})
	if err != nil {
		t.Fatalf("build failed: %v", err)
	}
...
func TestBuildGELFMessageInvalidJSONReturnsError(t *testing.T) {
	...
	if _, err := buildGELFMessage("svc", "info", payload, nil, NetworkConfig{Type: NetworkGraylogUDP, Source: "pod-1"}); err == nil {
