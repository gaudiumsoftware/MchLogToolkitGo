...
func TestDatagramGZIPDisabled(t *testing.T) {
	...
	if err := Configure(DestinationConfig{Protocol: ProtocolGraylogUDP, Addr: addr, Source: "pod-1", DisableGZIP: true}); err != nil {
		t.Fatalf("Configure: %v", err)
	}
	if err := Initialize("/applog/svc/"); err != nil {
		t.Fatalf("Initialize: %v", err)
	...
	defer conn2.Close()
	if err := Configure(DestinationConfig{Protocol: ProtocolGraylogUDP, Addr: addr2, Source: "pod-1", DisableGZIP: true}); err != nil {
		t.Fatalf("Configure: %v", err)
	}
	_ = MchLog.Close()
	if err := Initialize("/applog/svc/"); err != nil {
